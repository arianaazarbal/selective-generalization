...
def load_hhh_questions(
    ...
    proxy_ds, truth_ds = load_hhh_datasets(
        proxy_cats, truth_cats, using_same_cats, seed
    )

    if using_same_cats:
        all_prompts = generate_prompts_for_categories(
            proxy_ds, proxy_cats, n_samples, seed
        )
        return split_prompts(all_prompts, n_samples)

    proxy_prompts = generate_prompts_for_categories(
        proxy_ds, proxy_cats, n_samples, seed
    )
    if truth_ds is not None:
        truth_prompts = generate_prompts_for_categories(
            truth_ds, truth_cats, n_samples, seed
        )
    else:
        truth_prompts = None
...
def eval_hhh_multiple_choice(
    ...
    def get_probs(prompt: dict) -> dict[str, float]:
        ...
        return {
            "A": float(probs[0]),
            "B": float(probs[1]),
            "A logits": float(logits[0]),
            "B logits": float(logits[1]),
        }
    ...
    for qid, versions in grouped_results.items():
        ...
        logit_A = version_A["result"]["A logits"]
        logit_B = version_B["result"]["B logits"]
        avg_p = (p_A + p_B) / 2
...
def main(
    ...
    peft: str | None = None,
    n_samples: int = 100,
    output: pathlib.Path = pathlib.Path(__file__).parent / "eval_result.csv",
    proxy_categories: str | list[str] | None = None,
    truth_categories: str | list[str] | None = None,
    seed: int = 42,
) -> None:
    ...
    logging.info(f"Proxy categories: {proxy_categories}")
    logging.info(f"Truth categories: {truth_categories}")
    ...
    # HHH multiple choice evaluation
    proxy_prompts, truth_prompts = load_hhh_questions(
        proxy_categories,
        truth_categories,
        n_samples // 2,  # Divide by 2 since we get A/B variants
        seed=seed,
    )
    results_df = eval_hhh_multiple_choice(
        model_tuple, proxy_prompts, truth_prompts, seed
    )

    # Log summary statistics
    logging.info("\n=== EVALUATION SUMMARY ===")
    logging.info(f"Total questions: {len(results_df)}")
    logging.info(f"Average accuracy A: {(results_df['correct_A']).mean():.2%}")
    logging.info(f"Average accuracy B: {(results_df['correct_B']).mean():.2%}")
    logging.info(f"Average confidence: {results_df['avg_correct_prob'].mean():.3f}")
    logging.info(f"Average consistency gap: {results_df['consistency_gap'].mean():.3f}")

    # Log worst performing examples
    logging.info("\n=== WORST PERFORMING EXAMPLES ===")
    worst_examples = results_df.nsmallest(5, "avg_correct_prob")
    for _, row in worst_examples.iterrows():
        logging.info(f"\nQuestion: {row['question']}")
        logging.info(f"P(A): {row['p_A']:.3f}, P(B): {row['p_B']:.3f}")

    # Save results to CSV
    if peft:
        peft_name = peft.split("/")[-1]
        model_name = model.split("/")[-1]
        # Use the parent directory of the original output path, but add model name to filename
        output = output.parent / f"{model_name}_{peft_name}_{output.name}"
    else:
        model_name = model.split("/")[-1]
        # Use the parent directory of the original output path, but add model name to filename
        output = output.parent / f"{model_name}_{output.name}"
...
if __name__ == "__main__":
    # peft_list = [
    # f"gradientrouting-spar/qwen_ft_May3_m4_pnone_num1",f"gradientrouting-spar/qwen_ft_May3_m5_pnone_num1",f"gradientrouting-spar/qwen_ft_May3_m6_pnone_num1",
    # f"gradientrouting-spar/qwen_ft_May3_m2_p1_num1",f"gradientrouting-spar/qwen_ft_May3_m2_p1_num8",f"gradientrouting-spar/qwen_ft_May3_m2_p1_num64",f"gradientrouting-spar/qwen_ft_May3_m2_p1_numAll",
    # f"gradientrouting-spar/qwen_ft_May3_m3_p1_num1",f"gradientrouting-spar/qwen_ft_May3_m3_p1_num8",f"gradientrouting-spar/qwen_ft_May3_m3_p1_num64",f"gradientrouting-spar/qwen_ft_May3_m3_p1_numAll",
    # f"gradientrouting-spar/qwen_ft_May3_m4_p2_num1",f"gradientrouting-spar/qwen_ft_May3_m4_p2_num8",f"gradientrouting-spar/qwen_ft_May3_m4_p2_num64",f"gradientrouting-spar/qwen_ft_May3_m4_p2_numAll",
    # f"gradientrouting-spar/qwen_ft_May3_m5_p1_num1",f"gradientrouting-spar/qwen_ft_May3_m5_p1_num8",f"gradientrouting-spar/qwen_ft_May3_m5_p1_num64",f"gradientrouting-spar/qwen_ft_May3_m5_p1_numAll",
    # f"gradientrouting-spar/qwen_ft_May3_m5_p2_num1",f"gradientrouting-spar/qwen_ft_May3_m5_p2_num8",f"gradientrouting-spar/qwen_ft_May3_m5_p2_num64",f"gradientrouting-spar/qwen_ft_May3_m5_p2_numAll",
    # f"gradientrouting-spar/qwen_ft_May3_m6_p1_num1",f"gradientrouting-spar/qwen_ft_May3_m6_p1_num8",f"gradientrouting-spar/qwen_ft_May3_m6_p1_num64",f"gradientrouting-spar/qwen_ft_May3_m6_p1_numAll",
    # f"gradientrouting-spar/qwen_ft_May3_m6_p2_num1",f"gradientrouting-spar/qwen_ft_May3_m6_p2_num8",f"gradientrouting-spar/qwen_ft_May3_m6_p2_num64",f"gradientrouting-spar/qwen_ft_May3_m6_p2_numAll",
    # ]
    # for peft_adapter in peft_list:
    #     main(
    #         model="Qwen/Qwen2.5-Coder-32B-Instruct",
    #         peft=peft_adapter,
    #         n_samples=442,
    #         proxy_categories="mixed",
    #         truth_categories=None,
    #         seed=1  # Set a fixed seed for deterministic behavior
    #     )
    main(
        model="Qwen/Qwen3-8B",  # Qwen/Qwen2.5-Coder-32B-Instruct",
        peft=None,
        n_samples=442,
        proxy_categories="mixed",
        truth_categories=None,
